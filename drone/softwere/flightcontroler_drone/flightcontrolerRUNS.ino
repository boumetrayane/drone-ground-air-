...
<body>
  ...
  <script>
    let roadMode = false;
    let lastThrottle = 1000; // Default to 1000
    let lastThrottleX = 0;
    let lastThrottleY = 30;  // Bottom position (1000)

    // Set initial throttle to 1000 on page load
    window.onload = () => {
    ...
    joystickLeft.addEventListener('mousemove', (e) => {
      ...
      const throttle = Math.round((-y / 60) * 500 + 1500); // Invert Y: up = 2000, down = 1000
...
void loop() {
  ...
  //Serial.printf("Control Values - Throttle: %d, Roll: %d, Pitch: %d, Yaw: %d\n", throttleValue, rollValue, pitchValue, yawValue); 
  ...
  complementaryAngleRoll = 0.991 * (complementaryAngleRoll + RateRoll * 0.004) + 0.009 * AngleRoll;
  complementaryAnglePitch = 0.991 * (complementaryAnglePitch + RatePitch * 0.004) + 0.009 * AnglePitch;
  ...
  // Debugging
  Serial.printf("Throttle: %d, Roll: %d, Pitch: %d, Yaw: %d\n", throttleValue, rollValue, pitchValue, yawValue);
